...
std::string rmsnorm_cl_kernel_fp16_ =
  R"(
    ...
    int n = global_id / (H * C);       // Batch index
    int hc = global_id % (H * C);      // Combined height and channel index
    int h = hc / C;                    // Height index
    int c = hc % C;                    // Channel index
...
void RMSNormLayerCl::rmsnormProcess(Tensor const &input, Tensor &result,
  ...
  do {
    ...
    const int work_groups_count[3] = {b * h * c, 1, 1};
...
void RMSNormLayerCl::rmsnormProcess_fp16(Tensor const &input, Tensor &result,
  ...
  do {
    ...
    ret = RMSNormLayerCl::kernel_rmsnorm_fp16.SetKernelArguments(0, &inputbuf,
                                                                 sizeof(cl_mem));
    if (!ret) {
      break;
    }
    ret = RMSNormLayerCl::kernel_rmsnorm_fp16.SetKernelArguments(1, &resultbuf,
                                                                 sizeof(cl_mem));
    if (!ret) {
      break;
    }

    ret = RMSNormLayerCl::kernel_rmsnorm_fp16.SetKernelArguments(2, &gammabuf,
                                                                 sizeof(cl_mem));
    if (!ret) {
      break;
    }
